...
each array can only be accessed using packet fields, and repeated accesses to the 
same array is allowed only if the given field is unmodified between accesses.
...
\begin{table}
  \begin{tabular}{p{0.95\columnwidth}}
   ...
   At most one location in each array is accessed in a transaction. \\
   %Arrays are accessed A single execution of a transaction accesses only one entry in an array.\\
