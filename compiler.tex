...
Formally speaking, an atom template defines a constrained space of possible
computations (such as different ALU operations, or different sequences of N
instructions).  The exact computation is decided by \textit{configuring} the
atom. The codelet is a specification that needs to be pattern matched to one of
these computations by appropriately configuring the atom. In other words, we
need to \textit{synthesize} the atom's \textit{configuration}, given an
\textit{atom template} describing the atom's functionality.
