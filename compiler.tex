...
\label{fig:ssa}
\end{figure*}
\footnotetext{We rename all assignments, not just reassignments (such as the
assignment to pkt.id) because reads could precede these assignments.}
...
\end{figure*}
\footnotetext{The assignments to pkt.id and pkt.saved\_hop are flipped
  because pkt.id is an array index expression and is moved into
the read flank.}
...
% TODO: Write around constants in SKETCH.
