...
lexer and parser catch several errors with no additional effort. Using C also
allows us to use the macro preprocessor for constants. Finally, intrinsic
functions representing hardware primitives (e.g.  hashes and checksum), can be
implemented using arbitrary C code and linked with \pktlanguage code before
running the resulting binary on our abstract machine.
...
only the if and else constructs alter control flow in \pktlanguage; all other
control transfer (break, continue, loops) is forbidden.  This transformation
creates straight-line code, where control passes sequentially without
branching. Straight-line code simplifies the rest of the compiler, as we will
show (\S\ref{ss:ssa}).
...
only one array address is accessed by each packet in valid \pktlanguage
...
only support reads and writes, and all arithmetic happens on packet variables.
Restricting the operations on state variables simplifies handling of state
variables during code partitioning (\S\ref{ss:partitioning}).
...
there are no Write-After-Read or Write-After-Write dependencies; only
...
We next transform into three-address code~\cite{tac}, where all instructions are either
...
three-address code, we flatten expressions that are not already legal in
...
\absmachine instance~(\S\ref{ss:code_gen}), returning a compiler error if any
...
If the \absmachine instance supports complex atoms beyond three-address
