...
Using SKETCH for code generation frees the compiler developer from implementing
a custom code generator for every new \absmachine machine.  Instead, the
compiler developer only has to express the new \absmachine machine's
atom templates in SKETCH, which could then generate code for the new
\absmachine machine as well. 
