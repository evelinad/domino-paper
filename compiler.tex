...
sequential block of three-address code statements. 
%We do so by exploiting
...
Each codelet is then mapped to an atom provide by a particular
\absmachine machine~(\S\ref{ss:code_gen}), or an error is returned if it cannot
be mapped.
...
\begin{CompactEnumerate}
  ...
  \item Create a bidirectional edge between nodes N1 and N2, where N1 is a read from a
    ...
    instructions besides reads and writes, this suffices to handle state variables. \ac{what dos 'suffice' mean?}
  \item Create an edge (N1, N2) for every pair of nodes N1, N2 where N2 reads
    ...
    control dependencies turn into data dependencies after if conversion, and all
    other dependencies are removed after converting the code into SSA.
  ...
  \item Generate strongly connected components (SCCs) of the resulting graph
    (Figure~\ref{fig:partitioning_before}) and condense the SCCs to create a directed
    acyclic graph (DAG) (Figure~\ref{fig:partitioning_after}). This step captures the
    constraint that all operations on a given state variable
    must reside within the same atom as state is distributed across them.
  \item Schedule the resulting DAG using critical path
    ...
    \ac{what does 'operation' and 'follow' mean?}
\end{CompactEnumerate}
The resulting codelet pipeline\footnote{We refer to this both as a codelet
and an atom pipeline because codelets map one-to-one atoms
(\S\ref{ss:code_gen}).} shown in Figure~\ref{fig:flowlet_pipeline} 
implements the flowlet packet transaction.  Further, the
...
state variable and a write to a state variable, with optionally one
...
each for the multiply and accumulate), although one already suffices.\ac{say something
...
is not apparent whether such codelets can be mapped to an available atom. We
develop a new technique to determine the implementability of such stateful
codelets. 
%given as input the stateful atom template provided by the \absmachine
%machine.
First, each atom template is parameterized, where
the parameters determine the actual functionality provided by the atom. 
For instance, Figure~\ref{fig:alu_diag} shows a hardware circuit
that is capable of performing stateful addition or subtraction, depending
on the value of the constant and which output is selected from the multiplexer.
Its template representation is shown in Figure~\ref{fig:alu_in_sketch}, where
{\tt choice} and {\tt constant} represent the tunable parameters.
Each codelet can be viewed as a functional specification of the atom.
With that in mind,
the mapping problem is equivalent to searching for the value of the 
parameters to
configure the atom such that it implements the provided specification.

While many algorithms can be used to perform the search, in the \pktlanguage compiler
we use the SKETCH program synthesizer~\cite{sketch_asplos} for this purpose, as
the atom templates can be easily expressed using SKETCH, while SKETCH also provides
efficient search algorithms and has been used for similar purposes across
different domains~\cite{bitstreaming, lifejoin, qbs}.
As an illustration, 
assume we want to map the codelet {\tt x=x+1} to the atom template
shown in Figure~\ref{fig:alu_in_sketch}. The \pktlanguage compiler feeds in the codelet and 
the atom template into SKETCH, and SKETCH will search for possible values of 
the parameters such that the resulting atom performs the same functionality as 
the codelet, for all possible input values of {\tt x}.
In this case this is done by setting {\tt choice=0} and {\tt constant=1}.
In contrast, if the codelet {\tt x=x*x} was supplied as the specification, SKETCH
will return an error as no such parameter value exists. To minimize search time, 
the range of possible inputs and parameter values need to be specified in the 
template (e.g., all 8 bit integers), and our experiments have shown that 
the search finishes quickly, taking XXX secs at most.

\begin{figure}[!b]
  \begin{center}
  \includegraphics[width=0.4\columnwidth]{sketch.pdf}
  \caption{Overview of SKETCH and its application to atom configuration \ac{remove if out of space}}
  \label{fig:sketch}
  \end{center}
\end{figure}
...
\begin{figure}[h]
  ...
  \end{lstlisting}
  \caption{Circuit representation as a SKETCH template.}
