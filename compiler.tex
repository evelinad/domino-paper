...
literature~\cite{muchnik}. The \absmachine architecture, however, poses unique
challenges for compilation requiring a synthesis of techniques that, to the
best of our knowledge, hasn't been done before. Further, as we illustrate
...
\pktlanguage's compiler relative to mainstream compilers.  Since \pktlanguage's
syntax is effectively a subset of C, we use Clang's library
...
flows.  The conversion effectively turns the body of packet transactions into
...
variables before they can be operated.  Restricting operations on state
variables simplifies subsequent code partitioning (\S\ref{ss:partitioning}).

\subsection{Converting to static single-assignment form}
\label{ss:ssa}
...
redundant temporaries that compute the same subexpression, such temporaries
are removed using common subexpression elimination~\cite{cse}.
...
\begin{CompactEnumerate}
  ...
  \item Create an edge (N1, N2) for every pair of nodes N1, N2 where N2 reads a
    ...
    write-after-read and write-after-write dependencies are removed after
    converting the code into SSA.
  \item Generate strongly connected components (SCCs) of the resulting graph
    ...
    step captures the constraint that all operations on a given state variable
    must reside within the same atom.
...
manipulate state contain exactly one three-address code instruction after
...
each for the multiply and accumulate), although one already suffices.
