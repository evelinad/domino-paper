...
performance simplifies the \pktlanguage compiler.  Since \pktlanguage's syntax
is effectively a subset of C, we use Clang's library interface~\cite{libclang}
to parse and implement the passes in the compiler.  The overall architecture of
the compiler is shown in Figure~\ref{fig:passes}.  Throughout this section, we
use flowlet switching from Figure~\ref{fig:flowlet_code} as a running example
to demonstrate compiler passes.  While we have simplified the code for
...
constant and which output is selected from the multiplexer.  Its template
representation is shown in Figure~\ref{fig:alu_in_sketch}, where {\tt choice}
...
purposes across different domains~\cite{bitstreaming, lifejoin, qbs}.
...
the template (e.g., all 8 bit integers), and our experiments have shown that
the search finishes quickly, taking 10 secs at most.
