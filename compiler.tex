...
transform branches into the conditional operator \ac{conditional assignments sound 
better and less confusing than conditional statements}, starting from the innermost
...
makes it easier to map them to atoms during code generation (\S\ref{ss:code_gen}).
...
\begin{CompactEnumerate}
  ...
  \item Now, add {\em stateless dependencies} by adding an edge from any node
    ...
    write-after-read and write-after-write dependencies don't exist after SSA, as
...
width violations \ac{what does width mean again? Number of pipeline stages?}.  
If we violate the pipeline width, we insert as many new
...
other domains~\cite{bitstreaming, chlorophyll}. \ac{Sketch is not used in 
chlorophyll. Can you cite my PLDI 13 paper instead on QBS?}  As an
