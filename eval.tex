...
% Stress that we use a real transistor technology node here.
%Need to decide on two different aspects
%1. Computational limits: atom definitions.
% --> Disclaim how it's not easy to break the chicken-or-egg problem but we
%  have to start somewhere.
% --> Distinguish stateful from stateless atoms (refer Section 2).
% --> Stateless is easier to design, basically every 3-operand code instruction (meets timing at 1 GHz).
% --> Stateful depends on what algorithms you want to run and you need to break the cycle between the algorithm
%     and the instructions somewhere. We do this by inspecting the codelet pipelines and extracting generic circuits
%     from them that map a subset of algorithms (this isn't very clear).
% --> These all meet timing at 1 GHz and have area overheads of ...
%2. Resource limits: number of atoms per stage and number of stages.
% --> How do we size these?
% --> Use RMT paper as a rough guideline.
% --> Start with a 200 mm^2 chip.
% --> Dedicate about 7% to stateless atoms, verify we get a number similar to RMT.
% --> How many stateful atoms? Besides area, memory partitioning is a much bigger issue
%     and ends up being the bottleneck.
%     (private conversations indicate O(10) per stage is all that's doable).
%============================================ OLD text below ==============================
\subsection{Compiler targets}
