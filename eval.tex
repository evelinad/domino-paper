...
\begin{table}[!t]
  ...
  \begin{tabular}{|p{0.13\textwidth}|p{0.22\textwidth}|p{0.04\textwidth}|}
    \hline
    Atom & Description & Area ($\mu$ m\textsuperscript{2})\\
    \hline
...
%\hline
CoDel~\cite{codel} & \pbox{0.34\textwidth}{Update:\\Whether we are marking or not.\\Time for next mark.\\Number of marks so far.\\Time at which min. queuing delay will exceed target.}& Doesn't map & 15, 3 & Egress & 57 & 271\\
\hline
\end{tabular}
...
further evidence that \pktlanguage's programming model is convenient.
...
atom, assuming a chip area of 200 mm\textsuperscript{2} (the smallest area given by Gibb et.
al~\cite{gibb_parsing}), and an acceptable overhead of 7\% (the area overheads
for actions in RMT~\cite{rmt}), we can support ~10000 stateless atoms, given
the area of 1384 $\mu$ m\textsuperscript{2} per instance.  If these 10000 atoms were to be
spread across the same number of stages as RMT (32), we could support up to
\~300 stateless atoms per stage.

%This number is comparable to the 7000 VLIW ALUs supported by RMT~\cite{rmt},
%which are similar in complexity to our stateless atom. 

A similar analysis for stateful atoms yields 70 stateful atoms per stage for
the most complex stateful atom (Pairs) with an area of 5997 $\mu$ m\textsuperscript{2}.
However, stateful atoms access memory banks storing state in each stage.
Providing 70 independent memory banks in each stage supporting one read and
write per clock is prohibitive. Further, given an overall stage memory budget,
...
data-plane algorithms we are interested in. The area overhead of these 10
stateful atoms is \~1\%.
...
this, we rely on RMT~\cite{rmt}, which estimates a crossbar area of 6 mm\textsuperscript{2}
for a 32-stage pipeline with 224 action units. Scaling this proportionally to
300 stateless atoms, we estimate a crossbar area of 8 mm\textsuperscript{2} for a 4\% area
overhead.
...
algorithms---far beyond a fixed-function switch today. We anticipate designs
for \absmachine machines will evolve as data-plane algorithms demand more of
the hardware.
...
From Table~\ref{tab:algos}, we note two lessons for designing programmable
switches.  First, atoms supporting stateful operations on a single state
variable are sufficient for several data-plane algorithms (Bloom Filters
through Adaptive Virtual Queue in Table~\ref{tab:algos}). Second, there are
algorithms that need the ability to update a pair of state variables
atomically. One example is CONGA, whose code we reproduce below:
\begin{verbatim}
...
have a cost.  A more expressive atom needs more gates and incurs longer signal
propagation delays. A longer propagation delay implies a lower clock frequency
(the inverse of propagation delay) and hence a lower line rate.  To quantify
...
increases with more complex atoms, but performance (the max. line rate it
can support in packets/s) decreases.}
\label{tab:perfprog}
\end{table}
