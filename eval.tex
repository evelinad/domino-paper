...
stateless operations. Also, for simplicity, the atom templates we consider only
...
containment hierachy: going from left to right in Table~\ref{tab:templates}),
each atom template is strictly more expressive than its predecessor to its left
and hence can express all algorithms that its predecessor could.

We now look at every combination of atom templates from
...
template.  Table~\ref{table:eval} presents our results as a matrix.

\subsection{Interpreting these results}
There are two ways to interpret these results. From the perspective of a
network programmer, the matrix in Table~\ref{table:eval} tells her whether a
...
programmers continuously push the boundaries of these chips with new
algorithms.  The larger takeaway from this evaluation is that programming in
