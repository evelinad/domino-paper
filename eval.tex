...
already available as blocks of imperative code; translating them to \pktlanguage syntax
was straightforward.
...
Click~\cite{click}, the Linux qdisc subsystem)  are programmed in languages
resembling \pktlanguage---lending further evidence that \pktlanguage's
programming model is convenient.
...
32 nm standard-cell library, all atoms meet timing at 1 GHz and their area
...
the area of 1384 $\mu$ m\textsuperscript{2} per instance.  If these 10000 atoms were
...
300 atoms, we estimate a crossbar area of 8 mm\textsuperscript{2} with a 4\% area
overhead.
...
algorithms---far beyond a fixed-function switch today. We anticipate
...
manipulates a single state variable. Second, there are algorithms that need the
ability to update a pair of state variables atomically. One example is CONGA,
