...
First, we evaluate \pktlanguage's expressiveness and ease of
...
for programmable scheduling~\cite{prog_sched_arxiv}.\MA{Need to add an
  example in Table 4.} In all these cases, the algorithms are
naturally available as blocks of imperative code; translating them to
\pktlanguage syntax was straightforward.
%TODO: Need to make an arXiv document for programmable scheduling.
...
required, and how they are parsed.\MA{Are we cheating here by saying
  that P4 needs the programmer to specify something like parsing while
  we don't? Aren't we just ignoring parsing, but we would need it for
  a full solution?}

Further, the same implementation is only guaranteed to be correct on a
...
benefit from it.\MA{This is a weird thing to say without actually
  showing the P4 code and explaining the bug. I don't think anyone
  reading this will know what you're talking about.}

% Keep this?
Ideally, we could repeat the same comparison with P4 for all other
algorithms.  \MA{``Perhaps in part due to the difficulty of manually
  writing stateful algorithms in P4, these algorithms don't have P4
  counterparts.'' Avoid speculative statements like this.}
...
\MA{The proper way to have done this would have been to implement each
  algortihm in P4 to the best of our abilities. Using autogenerated P4
  code to make a LOC comparison seems lazy. Frankly, it might be more
  convincing to show some snippets of P4 code relative to Domino, so
  people can qualitatively see how awkward it is to write a data-plane
  algorithm in P4.}
...
\begin{table}[!t]
  ...
  \end{tabular}
\end{scriptsize}
\caption{Minimum delay of an atom increases with the depth of its circuit. MUX
