...
% Write a summary section at the beginning talking about how all these together form the eval.
% Sec 1. Expressiveness of programs, comparison with P4's flowlet switching.
% TODO: Compare against some P4 programs, for instance, flowlet switching and HULA.
% Sec 2. Compiler targets.
% Sec 3. Compiling domino programs and compilation times.
% Sec 4. Performance-programmability tradeoffs
...
We evaluate \pktlanguage's expressiveness by expressing several data-plane
algorithms (Table~\ref{tab:algos}) (\S\ref{ss:expressiveness}). We then design
a set of \absmachine machines that serve as compiler targets for \pktlanguage.
We show that these machines are readily feasible in today's transistor
...
rate) and the target's performance (the absolute line rate it can support).
