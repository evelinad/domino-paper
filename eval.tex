...
Table~\ref{tab:templates}~(\S\ref{ss:compiler}).  Finally, we describe lessons
learned from our evaluation~(\S\ref{ss:lessons}).
...
be broken into multiple pipeline stages without violating
...
combining both stateless and staeful atoms for all our targets is at most 1.1\%---a
...
\begin{table}[!t]
  ...
  \end{tabular}
  \end{scriptsize}
  \caption{Atom areas and minimum delays in a 32-nm standard-cell
...
% TODO: Be consistent between timing, delay, end-to-end delay, critical path, etc.
...
range in minimum delay between the simplest and the most complex atoms. This increase
...
a circuit is at least as small as the reciprocal of this minimum delay, a more complex
atom results in a lower clock frequency and a lower line rate. Although all our
atoms have a minimum delay under 1 ns (1 GHz), it is easy to extend them
