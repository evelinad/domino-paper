...
\item If there is a Read-After-Write dependency where a state variable is
updated in one instruction and needs to written into a packet variable, we can
still merge these instructions by replicating the same state update operation
for the packet update operation as well and relying on the fact that both
