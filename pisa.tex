...
This section describes \absmachine, a machine model for programmable line-rate switches
that serves as the compiler target for \pktlanguage
programs. \absmachine is inspired by recent programmable
...
every clock cycle. An incoming packet is processed by each stage and
handed off to the next, until it exits the pipeline. Each stage processes
a packet every clock cycle, which in our case is 1 ns. \absmachine
...
match-action pipeline in \S\ref{s:transactions}). \absmachine does not model
packet parsing and assumes that packets arriving to it are already parsed.
...
\paragraph{Algorithms that manipulate packet payloads}
...
\paragraph{Algorithms that manipulate large amounts of state infrequently}
\absmachine is ideally suited to algorithms that carry out small
transformations on a small amount of state frequently, e.g., setting a bit or
incrementing a counter. Some algorithms run infrequently (every few
microseconds, as opposed to on every packet (nanosecond)), but either access
large quantities of state or carry out involved computations. An example would
be periodically scanning a list of numbers to find their average or
