...
(Figure~\ref{fig:alu_diag}).\MA{Perhaps we should move the example
  from Figures 11 and 12 to this section. Is there any reason to leave
  it all the way till page 8?} Atom templates allow us to create
...
%TODO: Rename abstract machine to machine model consistently.
%TODO: Atoms not only tell you the computational capabilities i.e. circuit
%, but also the pairs of state you can manipulate together.
% Limits on number of packet field inputs, number of immediate operands, and
% number of state variable inputs.
% Maybe what we have is fine. But explain why it's particularly hard to handle
% arrays.
