...
packet every clock cycle (1 ns) and handes it off to the next, until it exits
...
when it should be 2. Locks over the shared counter object are a potential
...
clock cycle.\footnote{Wait-free objects~\cite{herlihy_wait} are an alternative
  ...
  still suffer from occasional pipeline stalls, which militates against line-rate
  performance.
...
interface between the compiler and the switch hardware.
...
atomicity by using one atom to add fields f2 and f3 in one pipeline stage
(clock cycle), and then using another atom to subtract f4 from the result in
the next, without having to provide a larger atom that supports the entire
operation.
...
consisting of each byte in the payload. Parsing deep parse graphs is
challenging at line rates of 1 GHz. Such processing is best left to
general-purpose CPU platforms~\cite{e2, aplomb, opennf}. Some algorithms
require complex computations, but not on every packet.  For example, consider a
