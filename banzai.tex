...
Unlike stateful atomic operations such as a counter---stateless atomic
...
f4 from the result in the next, without designing one large atom that
supports the entire operation.

%TODO: Maybe specify Hari's representation counter = counter ++ % 100 as well
...
\textbf{Resource limits:} We also need limit the number of atoms in each stage
...
tables per stage, memory per stage in programmable switch
